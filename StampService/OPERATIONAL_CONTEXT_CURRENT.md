...
- блок наград/истории реально переиспользуется из wallet UI через общий компонент `WalletBrandDetailsBlock`, поэтому изменения в визуальном паттерне наград применяются и в кошельке, и в карточке клиента;
