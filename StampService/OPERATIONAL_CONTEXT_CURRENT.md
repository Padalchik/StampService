...
- `POST /api/metrics/{metricDefinitionId}/issue-by-phone` - основной web-friendly сценарий выдачи метрики по номеру телефона клиента. Endpoint резолвит клиента через активную `Phone` identity в `IRecipientResolver`, затем вызывает `IssueMetricCommand`;
...
- `POST /api/brands/{brandId}/coins/issue-by-phone` - основной web-friendly сценарий начисления монеток по номеру телефона клиента;
