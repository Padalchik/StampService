...
## Demo reward notifications

Актуально на 2026-06-17.

Bugfix: при создании пользовательских демо-данных через `CreateUserDemoDataCommand` награды начислялись в ledger, но клиент не получал Telegram-уведомления о начислении. Причина была в архитектурном обходе обычных Application handlers: `CreateUserDemoDataHandler` использовал низкоуровневые `ICoinLedgerService` и `IMetricLedgerService` напрямую, а обычные `IssueCoinsByPhoneHandler` / `IssueMetricByPhoneHandler` после успешного ledger-шага вызывают `ICustomerNotificationService`.

Текущее правило: demo-flow может создать стендовые продукты, метрики, балансы и историю через ledger-сервисы, но все успешные demo-начисления клиенту должны проходить через общий порт `ICustomerNotificationService`. Это сохраняет одинаковое поведение web, API и Telegram: если у клиента есть активная Telegram identity и настроена доставка, он получает сообщение о начислении так же, как при обычной операции сотрудника.

Фикс находится в `src/StampService.Application/Demo/Commands/CreateUserDemoData/CreateUserDemoDataHandler.cs`: handler строит `CoinOperationResponse` / `IssueMetricResponse` из результатов ledger-операций и вызывает `NotifyCoinsIssuedAsync` / `NotifyMetricIssuedAsync` после успешных issue-шагов. Списания внутри demo-сценария остаются ledger-историей и не являются обязательной частью этого notification-fix, чтобы demo-fill не превращался в лишнюю серию клиентских сообщений о стендовых списаниях.

Regression coverage добавлен в `Tests/StampService.ApplicationTests/Demo/CreateUserDemoDataHandlerTests.cs`: demo-fill должен вызвать customer notification service для issue-операций по монеткам и метрикам.
