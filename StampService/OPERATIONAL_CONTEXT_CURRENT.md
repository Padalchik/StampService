...
- для draft-клиента блок `Операции` не рендерится. В карточке доступны только действия создания: `Создать клиента` и в будущем `Создать клиента и выдать приветственные награды`, если такая возможность будет включена настройками бренда;
- приветственные награды пока не реализованы как отдельный backend/Application flow и не имеют явного контракта в `BrandWorkspaceResponse`. До появления такого контракта UI не должен показывать форму настройки или ввода welcome-наград; будущая кнопка создания с приветственными наградами должна быть условной и появляться только по настройкам бренда;
