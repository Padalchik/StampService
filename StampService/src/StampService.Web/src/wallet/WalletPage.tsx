...
import { openUserWallet, type UserWalletBrandOverviewResponse, type UserWalletResponse } from './walletApi';
        ...
        {!wallet || wallet.brands.length === 0 ? (
          ...
          <div className="wallet-brand-list">
            {wallet.brands.map((brand) => (
              <WalletBrandCard key={brand.brandId} brand={brand} />
            ))}
          </div>
        )}
      </section>
...
function WalletBrandCard({ brand }: { brand: UserWalletBrandOverviewResponse }) {
  ...
  return (
    <article className="wallet-brand-card">
